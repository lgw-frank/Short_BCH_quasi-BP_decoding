...
class Decoder_Layer(tf.keras.layers.Layer):
    def __init__(self,initial_value = 0.421):
        ...
        self.H = self.code.H
        self.supplement_matrix =  1-self.H
        self.initials = initial_value
        self.noise_standard_variance  = np.sqrt(1. / (2 * (float(self.code.k)/float(self.code.n)) * 10**(GL.get_map('snr_lo')/10)))
    ...
    def call(self,inputs):
        ...
        if GL.get_map('selected_decoder_type') in ['SPA']:
            soft_input = 2*inputs[0]/(self.noise_standard_variance**2)
        else:
            soft_input = inputs[0]
...
# builds a belief propagation TF graph
    ...
    def compute_cv1(self,vc_matrix):
        ...
        vc_matrix = vc_matrix+self.supplement_matrix
        vc_matrix = tf.where(abs(vc_matrix)>0,vc_matrix,1e-5)
        temp = tf.reduce_prod(vc_matrix,2,keepdims=True)*self.code.H                      
    ...
    def compute_cv(self,vc_matrix):
        ...
        return cv_matrix
    def calculation_loss1(self,soft_output,labels,loss):
         #cross entroy
...
def training_block(start_info,Model,optimizer,exponential_decay,selected_ds,log_info,restore_info):
    ...
    if start_step < GL.get_map('termination_step'):
        for i in range(start_step,num_counter):
                with tf.GradientTape() as tape:
                    inputs = input_list[i]
                    soft_output_list,loss,label = Model(inputs)
                    fer,ber,_,_= Model.get_eval1(soft_output_list,label)
