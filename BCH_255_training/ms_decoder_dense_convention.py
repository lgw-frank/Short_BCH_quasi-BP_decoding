...
class Decoding_model(tf.keras.Model):
    ...
    def get_eval(self,soft_output_list,labels):
        ...
        return FER, BER,len(diff_set),index1   
...
# builds a belief propagation TF graph
    ...
    def belief_propagation_op(self,soft_input, labels):
        ...
        return tf.while_loop(
            ...
            loop_vars = [
                ...
                labels,
                0, # iteration number
                tf.zeros([soft_input.shape[0],self.code.check_matrix_row,self.code.check_matrix_col],dtype=tf.float32)    ,# cv_matrix
                0.,# loss
    ...
    def calculation_loss(self,soft_output,labels,loss):
        ...
        return CE_loss+loss
    #common sense definition taking into account all bits of codewords         
    def calculation_ber(self,soft_output,labels):
        hard_decision = tf.cast(tf.where(soft_output>0,0,1),tf.int64)
        cmp_result = tf.where(hard_decision==labels,0,1)
        BER = tf.reduce_sum(cmp_result)/(cmp_result.shape[0]*cmp_result.shape[1])
        return BER
    ...
    def belief_propagation_iteration1(self,soft_input, labels, iteration, cv_matrix, loss,soft_output_list):
        # compute vc
    ...
    def belief_propagation_iteration(self,soft_input, labels, iteration, cv_matrix, loss,soft_output_list):
        ...
        soft_output = self.marginalize(cv_matrix, soft_input,iteration,soft_output_list) 
        # if iteration == 0:
        #   current_ber = self.calculation_ber(soft_output_list[0],labels)
        #   tf.print(f'iter:{iteration}Batch BER:{current_ber:.3f}')
        iteration += 1   
        #current_ber = self.calculation_ber(soft_output,labels)
        #tf.print(f'iter:{iteration}Batch BER:{current_ber:.3f}')
        loss = self.calculation_loss(soft_output,labels,loss)
        return soft_input, labels, iteration, cv_matrix, loss,soft_output_list
...
def postprocess_training(Model,iterator):
    ...
    for i in range(num_counter):
        ...
        FER,BER,_,indices= Model.get_eval(soft_output_list,label)
    ...
    return buffer_inputs,buffer_labels,FER_sum/(num_counter-1),BER_sum/(num_counter-1),num_counter
    ...
    while True:
        for i in range(start_step,num_counter):
                with tf.GradientTape() as tape:
                    inputs = input_list[i]
                    soft_output_list,label,loss = Model(inputs)
                    fer,ber,_,_= Model.get_eval(soft_output_list,label)
