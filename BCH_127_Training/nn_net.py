...
class Convention_NMS_model(tf.keras.Model):
    def __init__(self):
        super().__init__()
    ...
    def get_eval(self,soft_output_list,labels):
        ...
        index = np.nonzero(tf.reduce_sum(syndrome,-1))[0]
        err_batch = tf.where(tmp == labels,0,1)
        err_sum = tf.reduce_sum(err_batch,-1)
        ...
        #identify the indices of undected decoding errors        
        return FER_count, BER_count,index    
...
class Decoder_Layer(tf.keras.layers.Layer):
    ...
    def belief_propagation_op(self, soft_input, labels):
        ...
        cv_matrix = tf.zeros([soft_input.shape[0],self.code.H.shape[0],self.code.H.shape[1]],dtype=tf.float32)# cv_matrix
